...
이번 포스트에는 라우터를 이용해서 WAN(인터넷)을 구성해 보고 데이터가 목적지까지 어떻게 이동하는지 자세히 알아보자. 가장 먼저 집이나 회사 같은 작은 LAN영역을 허브와 스위치를 이용해 구성하자. 허브와 스위치로 만들어진 LAN영역은 MAC주소만을 이용해 통신이 가능하므로 이렇게 연결된 컴퓨터들은 통신할 수 있다. 이와 같은 네트워크는 세계 곳곳에 셀 수 없을 만큼 많이 있을 것이다. 그중에 짱구와 철수는 LAN영역 3에 있다고 하자. LAN영역을 넘어서 통신하기 위해서는 라우터가 필요하다. 라우터를 설치하고 우리의 네트워크는 10.0.0.0으로 할당받았다고 하자. 10.0.0.0/8은 클래스 A로 1600만 개의 IP를 할당할 수 있는 아주 거대한 네트워크이므로 실제로는 이렇게 할당될 일이 없지만 단순하게 생각하기 위해서 이렇게 할당을 받았다고 하자. 그리고 아직 NAT를 배우지 않았기 때문에 private IP는 사용하지 않고 public IP만 사용한다고 가정하자. 이제 이 네트워크의 호스트에 적절한 IP를 부여하자. 우리의 PC에는 10.0.0.2라는 IP주소가 할당되었다. 짱구와 철수가 포함된 네트워크도 똑같은 방식으로 IP가 할당되었다고 하자. 짱구와 철수가 속한 네트워크는 8.0.0.0으로 할당받았다고 하자. 짱구는 8.0.0.2, 철수는 8.0.0.3의 IP를 할당받았다. 같은 방식으로 세계 곳곳에 있는 LAN영역은 WAN을 연결하기 위해서 라우터를 설치하고 각각 IP를 할당했다. 이러한 라우터들은 서로 연결된다. 라우터끼리 연결될 때도 인터페이스(포트)마다 IP를 부여한다. 라우터들은 다른 네트워크로 접근하기 위한 경로를 라우팅 프로토콜을 이용해 메모리에 기억한다. 이것을 라우팅 테이블이라고 부른다. 이제 우리가 짱구에게 데이터를 전송하면 어떤 일이 벌어질까?

먼저 우리는 보낼 데이터에 출발지 IP는 우리의 IP주소인 10.0.0.2를, 목적지 IP는 짱구의 IP인 8.0.0.2를 적는다. 목적지 IP주소를 보니 다른 네트워크에 있는 호스트로 데이터를 전송하려는 것을 알 수 있다. 즉, 라우터를 거쳐야한다는 것이다. 따라서 출발지 MAC주소는 우리의 MAC주소를, 목적지 MAC주소엔 라우터 1 포트의 MAC주소를 적고 전송한다. 스위치에선 목적지 MAC주소를 보고 해당 MAC주소인 포트로 데이터를 전송한다. 그러면 라우터 1에 데이터가 도착하는데 라우터는 데이터의 목적지 IP주소를 보고 이렇게 말한다.

> 💻 8.0.0.2는 8.0.0.0/8에 속한 네트워크야! 8.0.0.0 네트워크는 직접 연결이 되지 않았으니 20.0.0.2(디폴트 라우트)로 이동!

그럼 출발지 IP와 목적지 IP는 변경하지 않고 MAC주소는 라우터1과 라우터 2가 연결된 인터페이스의 MAC주소를 적어준다.

> 실제로 라우터 전송은 이더넷 프로토콜보다는 HDLC나 PPD라는 데이터링크 프로토콜이 주로 사용되지만 이더넷으로 보낸다고 생각하는 게 이해가 쉽다.

이렇게 라우터2에 데이터가 도착하면 라우터는 목적지 IP 주소를 보고 이렇게 말한다.

> 💻 8.0.0.2는 8.0.0.0/8에 속한 네트워크야! 8.0.0.0 네트워크는 직접 연결이 되지 않았으니 30.0.0.2(디폴트 라우트)로 이동!

그럼 출발지 IP주소와 목적지 IP를 변경하지 않고 MAC주소는 라우터 2와 라우터 3이 연결된 인터페이스의 MAC주소를 적어준다. 이렇게 라우터 3에 데이터가 도착하면 라우터 3은 목적지 IP주소를 보고 이렇게 말한다.

> 💻 8.0.0.2는 나와 연결되어 있어!

자신과 연결된 네트워크인것을 알아차린 라우터 3은 출발지 IP와 목적지 IP를 변경하지 않고 출발지 MAC주소는 8.0.0.0 네트워크와 연결된 인터페이스의 MAC주소를 적어주고 목적지 MAC 주소는 짱구의 MAC주소를 적고 데이터를 전송한다. 그러면 스위치는 MAC주소를 보고 짱구의 PC로 정확하게 전송한다. 인터넷은 이렇게 동작한다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## 스태틱 라우팅

이번 포스트엔 라우팅 프로토콜의 한 종류인 스태틱(정적) 라우팅에 대해 알아보자. 스태틱 라우팅은 관리자가 라우터에 직접 목적지 네트워크 정보를 입력하는 방법이다. 나는 이전 포스트에 구성한 네트워크에서 라우터 1,2,3에 스태틱 라우팅으로 라우팅 테이블을 만들었다고 가정하자. 한번 라우터 1의 설정을 봐보자. 라우터 1의 라우팅은 9.0.0.0/8과 8.0.0.0/8 네트워크로 보내는 데이터는 20.0.0.2로 가라고 적혀있다. 이 말은 라우터 2로 전송하라는 뜻이다. 만약 다른 라우터가 물리적으로 연결은 되어 있다고 한다 해도 라우터에 경로를 적어두지 않아서 논리적으로는 끊어진 것과 같다. 그럼 우리의 PC에서 짱구에게 데이터를 전송한다면 내가 설정한 값대로 라우터 2에 보낸다. 여기서 라우터 2의 라우팅 설정을 살펴보자. 10.0.0.0 네트워크로 가는 것은 라우터 1, 8.0.0.0 네트워크로 가는 것은 라우터 3으로 설정되어 있다. 우리가 짱구에게 보낸 데이터는 라우터 3으로 이동한다. 스태틱 라우팅은 이렇게 사람이 직접 경로를 지정해 주기 때문에 라우터의 CPU가 계산할 것이 많지 않아 처리속도가 빠르고 메모리도 적게 사용하므로 라우터에 부담이 적다. 그리고 다이내믹 라우팅처럼 다른 라우터와 정보를 교환하지 않으므로 네트워크 대역폭을 절약할 수 있다. 또한 자신의 정보를 외부로 전송하지 않아서 보안이 더 좋다. 하지만 단점도 존재한다. 특정 라우터가 고장이 났거나 다른 문제가 생겼다면 통신자체가 안 될 수 있다. 이런 상황에서 다이내믹 라우팅은 우회해서 통신을 하지만 스태틱 라우팅은 그렇게 할 수 없다. 관리자가 다른 경로를 지정해 주거나 고장 난 라우터를 고쳐야만 한다. 그리고 네트워크가 커지면 관리자가 일일이 설정해 주기 힘들다. 따라서 대규모 네트워크에서 사용이 불가하다. 스태틱 라우팅의 이런 단점을 보완하기 위해서 다이내믹 라우팅이라는 것이 만들어졌다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## 다이내믹 라우팅

다이내믹 라우팅은 다시 여러 가지 프로토콜로 나눠진다. 인터넷상에서 라우터는 셀 수도 없을 만큼 많이 존재한다. 여기서 하나의 큰 조직이 많은 라우터를 연결해 만든 네트워크를 Autonomous System, 즉 AS라고 부른다. 이 조직들은 대학교, 회사, 통신사들이 될 수 있다. 우리나라의 AS정보는 한국인터넷진흥원에서 확인할 수 있다. 이렇게 AS가 있을 때 AS 내부에서 쓰는 라우팅 프로토콜을 Interior Routing Protocol(IRP)라고 부른다. 그리고 서로 다른 AS를 연결하기 위한 라우팅 프로토콜을 Exterior Routing Protocol(ERP)라고 부른다. IRP에 있는 라우팅 알고리즘은 동작에 따라서 다시 2가지 방식으로 구분되는데 한 가지는 Distance Vector방식이고 나머지 한 가지는 Link State방식이다. 우리는 대표적인 Distance Vector방식인 Routing Information Protocol(RIP)과 대표적인 Link State방식인 Open Shortest Path First(OSPF)를 알아볼 것이다. 그리고 ERP의 Path Vector방식인 Border Gateway Protocol(BGP)을 알아볼 것이다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## RIP

이번 포스트에는 AS 내에서 사용되는 동적 라우팅 프로토콜인 Distance Vector방식의 RIP를 알아보자. RIP는 목적지 라우터까지의 거리와 목적지까지 가기 위해서 어떤 인접 라우터 방향으로 가는지를 라우팅 테이블에 저장한다.

> 목적지 라우터로 가기 위한 인접 라우터의 방향과 거리를 저장

그리고 이 정보를 인접 라우터와 30초마다 공유하는 방식으로 라우팅 테이블을 유지한다. 한번 어떻게 동작하는지를 알아보자. LAN이 존재하고 라우터들이 각각의 LAN과 연결되어 있으며 이 라우터들이 다른 네트워크와 연결되는 각 포트에 IP주소를 할당받는다고 해보자. 라우터들은 또 다른 라우터들과 연결되어서 IP주소를 할당받았다고 하자. 초기엔 라우팅 테이블은 자신과 연결된 네트워크 정보를 담고 있다. 라우터 1번 입장에서는 20.0.0.0 네트워크가 자신과 연결되어 있으므로 홉은 0이다. 포트에 직접 연결되어 있다는 것이다. (Distance = 0) 여기서 30초가 지나면 라우터들은 인접한 라우터와 라우팅 테이블의 정보를 교환한다. 그러면 라우터 1의 라우팅 테이블은 기존 정보에 라우터 2의 정보가 추가된다. 20.0.0.0 네트워크는 이미 저장되어 있는 경로의 홉과 같은 값이므로 업데이트하지 않고 나머지 정보만 가져온다. 그리고 인터페이스는 라우터 2와 연결된 1번 포트로 바꾸고 홉도 1씩 증가한다. 라우터 1에서 라우터 2까지 홉을 하나 이동하기 때문에 홉 카운트를 1 올려주는 것은 당연한 것이다. 마찬가지로 라우터 2의 라우팅 테이블은 기존 정보에 라우터 1의 정보가 추가된다. 이렇게 라우터 1과 라우터 2가 정보공유를 하는 동시에 라우터 2와 라우터 3도 교환한다. 이때 중요한 것은 라우터 1과 라우터 2가 정보공유를 하는 동시에 라우터 2와 라우터 3도 교환이 동시에 일어나기 때문에 라우터 1의 정보는 라우터 3에 전달되지 않고 라우터 3의 정보도 라우터 1에 전달되지 않는 것이다.

> 🙋🏻 그러면 경로유지를 못하지 않나요? 엉터리 알고리즘이네요!

여기서 끝이 아니다. 다시 30초가 지나면 어떤 일이 발생할까? 라우터1과 라우터 2는 다시 정보교환을 한다. 라우터 1에는 변화가 없으므로 라우터 2의 라우팅 테이블은 업데이트되지는 않지만 라우터 2에는 변화가 발생하였다. 바로 라우터 3의 정보가 저장된 것이다. 따라서 라우터 1은 라우터 2의 업데이트된 정보로 테이블을 업데이트해야 한다. 여기서 라우터 2의 홉 카운트에서 1을 올려준다. 당연히 하나의 홉을 더 이동하는 것이니 홉을 올려주는 것이다. 이렇게 라우터 1이 업데이트되는 동시에 라우터 3도 업데이트된다. 라우터 3은 라우터 2에서 라우터 1의 정보를 가져온다. 이렇게 3개의 라우터가 1분 만에 모든 정보를 공유했고 완벽하게 경로를 찾을 수 있게 되었다.

라우팅 테이블에 변화가 생겼을 대 모든 라우터에 업데이트가 완료되는 시간을 'Coverage Time'이라고 부른다. 이 예시에선 최대 2단계의 라우터를 이동하기 때문에 Coverage Time이 1분이다.

> 🙋🏻 간단한 방식이라 효과적이네요!

맞다! RIP는 하나의 라우터가 모든 정보를 가지고 있지 않기 때문에 적은 메모리를 요구한다. 가장 큰 장점은 구성이 간단하다는 것이다. 따라서 표준으로 사용하는 곳이 많은 프로토콜이다. 하지만 라우팅 테이블의 변화가 없어도 30초마다 인접 라우터와 라우팅 테이블을 교환하기 때문에 쓸데없는 트래픽이 발생한다. 마치 운영체제에서 키보드 입력을 확인하려고 while문을 계속 돌리는 폴링방식을 사용하는 것처럼 말이다. 변화가 생겼을 때 인터럽트 방식처럼 효율적이지 못하다. 또한 업데이트된 라우팅 테이블의 정보가 하나의 홉을 건너기 위해서는 30초라는 시간이 걸린다. 만약 5홉을 건너야 하는 상황이라면 Coverage Time은 300초가 된다. 그리고 경로를 나타내는 정보가 다음 라우터와 홉 밖에 없는 Distance Vector 방식이므로 전체 경로를 알 수 없다. 따라서 순환경로에 빠지는 문제가 발생한다. 이런 이유로 RIP는 대규모 네트워크에서는 사용이 어렵고 소규모 네트워크에서 주로 사용되는 라우팅 프로토콜이다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## OSPF

이번 포스트엔 AS 내에서 사용되는 동적 라우팅 프로토콜의 Link State 방식인 OSPF를 알아보자. 각각의 라우터들은 자신과 연결된 모든 이웃 라우터에 LSA를 전달한다.

> 🙋🏻 LSA는 무엇인가요?

LSA란 Link State Advertisement의 줄임말로 자신의 링크 정보, 즉 연결정보를 광고하는 메시지다. LSA는 광고하는 라우터의 ID와 연결된 포트의 IP주소, 비용 등이 적혀있다. 여기서 비용이란, 연결된 케이블의 속도를 말한다. 속도가 빠를수록 비용이 적다. 만약 라우터 1이 LSA를 전송한다면 먼저 라우터2와 라우터 5에 도착한다. 라우터 1의 LSA를 받은 라우터2와 라우터 5는 이웃 라우터에 해당 LSA를 전달한다. 이때 중복으로 LSA를 받은 라우터는 늦게 도착한 LSA를 파기한다. 이렇게 라우터1의 LSA는 모든 라우터에 도착한다. 그럼 이 네트워크의 모든 라우터는 라우터 1이 라우터 2와 라우터 5에 연결되었다는 것을 알게 된다. 같은 방식으로 모든 라우터는 동시다발적으로 각자의 LSA를 연결된 모든 라우터에 전달한다. 전달된 LSA는 각 라우터의 Link State DataBase(LSDB)에 저장된다. 모든 라우터가 LSA를 교환한다면 모두 같은 LSDB를 가지게 된다. 그러면 이 LSDB를 이용해서 최단경로를 계산한다. 최단경로는 다익스트라 알고리즘을 이용해 계산한다.

OSPF는 이렇게 라우터의 정보(LSA)를 연결된 모든 라우터에 전달해 모든 라우터가 다익스트라 알고리즘을 이용해 각자 라우팅 테이블을 만든다. 따라서 RIP와는 다른 장단점이 존재한다. OSPF의 장점으로는 RIP와 다르게 한 라우터에서 변화가 생기면 다른 모든 라우터에 업데이트되는 시간이 짧다는 것이다. 즉, Coverage Time이 짧다는 것이다. 또한 이웃 라우터와 라우팅 테이블을 자주 교환하는 RIP와는 다르게 링크정보를 자주 교환하지 않아서 트래픽을 줄일 수 있다. 그리고 홉의 제한이 있지 않아서 대규모 네트워크에서 사용이 가능하다. 하지만 이렇게 장점만 있는 게 아니다. OSPF는 라우터가 모든 라우팅 정보를 저장하기 때문에 많은 메모리가 필요하고 최단경로도 계산해야 하므로 CPU도 더 바빠진다. 이 말은 라우터의 하드웨어 성능을 높여야 한다는 뜻이고 비용이 많이 들어간다는 뜻이다. 그리고 다른 라우팅 프로토콜보다 복잡해서 배우기 쉽지 않다는 점이 있다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## BGP

이번 포스트엔 AS 내에서 사용되는 것이 아닌 서로 다른 AS를 연결하기 위한 Path Vector 방식의 BGP를 알아보자.

> 🙋🏻 그런데 왜 굳이 AS를 나눠서 다른 프로토콜을 사용하나요?

어떤 그룹에 AS를 부여하고 나눠서 관리하면 여러 가지 이점이 생기기 때문이다. 첫 번째로 정책적인 이점이 있다. 서로 경쟁 중인 통신사가 있다면 이해관계에 따라서 자신들의 데이터를 다른 통신사로 전달할지, 하지 않을지를 컨트롤할 수 있다. 두 번째 이점은 라우터 집단을 구분했기 때문에 트래픽이 줄어든다는 점이다. 만약 구분하지 않았다면 한 단체의 하나의 라우터에 변화가 생겼을 때마다 그 정보가 다른 단체의 모든 라우터로 전달되어 트래픽이 많아질 것이다. 하지만 AS로 구분하면 외부와 연결된 라우터만 업데이트하면 되므로 트래픽이 감소하고 AS내부의 다른 라우터들은 다른 AS 내 라우터의 변화를 신경 쓰지 않을 수 있다.

그럼 BGP가 어떻게 동작하는지 알아보자. 라우터들로 연결된 영역을 AS1번부터 5번까지 나눴다고 하자. AS 내에는 수 많은 라우터와 네트워크가 더 존재할 테지만 지금은 1~2개만 있다고 생각하자. 각 AS에서 외부 AS와 연결되는 라우터를 Autonomous System Boundary Router, 줄여서 ASBR이라 부른다. 여기서 ASBR들은 서로 경로속성을 공유한다. 경로속성은 연결된 모든 ASBR과 양방향으로 교환하는데 지금은 R1이라는 ASBR이 R2에게 공유한 경로속성이 어떻게 흘러가는지 살펴보자. 경로속성에는 라우터의 우선순위 같은 값들이 존재하는데 여기선 간략하게 경로만 나타내는 값만 표현하겠다. 가장 먼저 R1은 10.0.0.0의 네트워크는 AS1에 있다는 정보를 R2에게 전달한다. R2에 도착한 이 정보는 R4로 전달되고 R4는 R5에게 10.0.0.0 네트워크와 9.0.0.0 네트워크로 가는 경로를 전달한다. 이때 중요한 것은 10.0.0.0 네트워크로 가는 경로에 자신의 경로, 즉 AS2를 추가된다는 것이다. 그럼 R5는 도착한 이 정보에 8.0.0.0까지 가는 경로를 더해 R6로 전달한다. R6에 전달된 이 정보는 R8에게 전달되고 R8은 24.0.0.0과 25.0.0.0 네트워크로 가능 경로를 추가해 R9에게 전달한다. R9은 R10에게 이 정보를 전달하고 R10은 받은 정보를 다시 R1에게 전달한다. 여기서 R1은 10.0.0.0 네트워크 정보는 다시 자신에게 오는 경로라 이 데이터를 초기화하고 자신의 AS1을 경로에 넣는다. 즉, 순환경로를 제거하는 것이다.

> 이 외에도 경로속성을 보고 더 빠르게 도착할 수 있는 경로를 선택

지금은 R1에서 R2방향으로 가는 경로 속성의 전달과정만 살펴봤는데 이 작업은 모든 라우터의 모든 방향으로 이루어진다. 그러면 모든 경로를 이동하는 경로표가 만들어져 AS간의 라우팅을 처리할 수 있다.

> 🙋🏻 AS 내부 경로는 모르는데 어떻게 전달해요?

AS2를 예시로 든다면 R2에서 R4로 가는 경로를 알아야 한다. EXP는 IRP와 함께 쓰이기 때문에 AS 내부 경로는 스태틱 라우팅 혹은 RIP나 OSPF로 이미 알고 있는 상태이다. ASBR은 BGP뿐만 아니라 스태틱 라우팅 혹은 RIP나 OSPF도 같이 사용한다는 뜻이다. 따라서 내부의 경로는 걱정하지 않아도 된다.
