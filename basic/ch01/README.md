...
정확한 이유는 찾지 못했지만 내 추측으로는 다음과 같은 상황 때문이라고 생각한다. UTP 케이블로 송수신을 동시에 하는 상황으로 허브에 UTP 케이블로 2대의 컴퓨터가 연결되어 있다고 가정하자. 여기서 PC1과 PC2가 서로에게 데이터를 동시에 전송한다면 각각의 데이터가 허브에 도착할 것이다. 허브는 PC1에게 받은 데이터를 PC1을 제외한 모든 PC로 유니캐스팅을 한다. 마찬가지로 PC2에게 받은 데이터는 PC2를 제외한 모든 PC로 유니캐스팅을 한다. 이렇게 UTP 케이블의 송수신선을 나눠서 전이중 통신이 성공적으로 이루어진다. 하지만 문제는 더 많은 PC가 연결된 상황에서 발생한다. 하나의 PC가 더 연결되어 PC1과 PC2가 동시에 데이터를 전송하는 상황으로 가정하자. 허브는 PC1에서 받은 데이터를 PC1을 제외한 모든 PC로 유니캐스팅을 한다. PC2와 PC3으로 말이다. 마찬가지로 PC2에서 받은 데이터는 PC2를 제와한 모든 PC로 유니캐스팅한다. PC1과 PC3으로 말이다. 여기서 PC3으로 가는 UTP 케이블의 수신선에 충돌이 발생한다. PC1에서 온 데이터와 PC2에서 온 데이터가 동시에 전송되니 충돌이 발생한 것이다.

> 🙋🏻 그러면 PC1 데이터를 먼저 보내고 PC2를 보내면 될 것 같은데요?

맞다. 그렇게 하면 충돌이 발생하지 않는다. 하지만 그렇게 하려면 나중에 보낼 데이터를 기억할 메모리와 처리장치가 필요한데 허브는 가지고 있지 않다. 허브는 단순히 들어온 신호를 모든 PC에게 유니캐스팅을 해주는 포트가 많은 리피터 역할이기 때문이다. 따라서 송수신선이 따로 있는 UTP 케이블이더라도 논리적으로 반이중 통신을 선택해야만 했다. 이후에 브리지, 스위치가 만들어지면서 조금 전에 말한 방법이 가능해졌고 1997년 IEEE 802.3x에서 전이중 통신 표준이 발표되었다. 드디어 UTP 케이블의 송수신선을 제대로 이용할 수 있게 되었다. 현재는 전이중 통신을 이용하기 때문에 CSMA/CD는 거의 사용하지 않는 고대유물로 생각하면 된다.
