...
허브를 알아 본 포스트에 CSMA/CD라는 용어가 등장했다. 한 순간에는 하나의 PC만 데이터를 전송한다고 말하고 넘어갔었다. 이번 포스트에는 CSMA/CD가 어떻게 생겨났고 동작하는지 알아보자.

우리는 허브를 통해서 여러 대의 컴퓨터가 통신할 수 있다는 것을 알고 있다. 이 구조는 어떤 네트워크 토폴로지일까? 바로 스타형 토폴로지이다. UTP 케이블을 허브에 연결해서 스타형 구조가 되었다. 우리는 CSMA/CD가 어떻게 탄생하게 되었는지 이해하기 위해서 허브가 생겨나기 이전으로 이동해 보자.

이더넷 초기에 가장 먼저 표준으로 나온 케이블은 10BASE5라는 동축 케이블이다. 1983년도에 발표되었다. 이때는 UTP케이블과 허브가 없었는데 어떻게 여러 대의 컴퓨터가 통신을 할 수 있었을까? 바로 하나의 동축 케이블에 버스형으로 연결하여 사용하였다.

> 🙋🏻 동축 케이블은 1개인데 어떻게 다른 컴퓨터까지 선을 연결할 수 있었나요?

바로 BNC T Connector라는 것을 이용하여 연결하였다. BNC T Connector에 동축 케이블들을 연결하여 사용하였다. 동축 케이블은 하나의 선으로 전기신호를 보내 통신을 하므로 2대 이상의 컴퓨터가 동시에 신호를 보내면 충돌이 발생하여 데이터가 손상된다. CSMA/CD는 이런 충돌을 방지하기 위해 만들어진 프로토콜이다.

CSMA/CD는 Carrier Sense Multi Access / Collision Detection의 약자이다. Carrier는 반도체 내에서 전류의 흐름을 발생시키는 녀석이다. Carrier Sense는 전선에 Carrier를 감지하는 것이다. 동축 케이블 내 Carrier가 많다면 전류가 흐르고 있다고 판단한다. Multi Access라는 말은 2대 이상의 컴퓨터가 동축 케이블에 접근하는 것을 말한다. 마지막으로 Collision Detection은 2대 이상의 컴퓨터가 접근해 충돌이 발생한 것을 탐지하는 걸 말한다.

그럼 예시를 보면서 CSMA/CD가 어떻게 이루어지는지 알아보자. 현재 네트워크는 버스형으로 구성되어 있고 아무런 통신이 없는 상태라고 가정하자. 여기서 PC1과 PC3은 PC2에게 데이터를 보내고 싶어 한다. PC1은 동축 케이블에 데이터가 전송되는지 확인한다. 즉 Carrier Sense를 한다. 마찬가지로 PC3도 Carrier Sense를 해서 데이터가 전송되는지 확인한다. 두 PC는 현재 동축 케이블에 데이터가 전송되지 않는 것을 확인했기 때문에 PC2로 데이터를 전송한다. 즉 Multi Access를 한 것이다. 하지만 데이터가 전송되다가 충돌이 발생했다. 여기서 충돌을 감지하는 것을 Collision Detection이라고 한다. 그러면 충돌을 감지한 컴퓨터는 데이터를 전송을 중지하고 연결된 모든 컴퓨터에게 충돌이 났다는 Jamming 신호를 전송한다. 데이터를 전송하고자 하는 컴퓨터는 랜덤한 시간 동안 기다렸다가 재전송한다. 예를 들어 PC1이 2초, PC3이 3초를 기다리고 재전송을 한다고 가정하자. PC1이 2초 뒤에 전송을 하고 PC3이 3초 뒤에 재전송을 하므로 충돌이 발생하지 않는다. 만약 기다렸다가 재전송할 때도 충돌이 발생한다면 또다시 랜덤시간 동안 기다렸다가 재전송을 하는데 충돌이 15번 반복하면 더 이상 전송하지 않고 전송을 포기한다. 이는 하나의 컴퓨터에서 고장이나 악의적인 목적으로 계속 데이터를 보낼 때 모든 컴퓨터가 통신이 불가능해지는 것을 막기 위함이다. 이렇게 CSMA/CD로 동축 케이블 하나를 이용해 모든 컴퓨터가 안전한 반이중 통신이 가능해졌다.

> 🙋🏻 허브를 설명할 때 UTP 케이블을 이용해서 연결했잖아요! UTP 케이블은 전송용, 수신용이 따로 있다면서 왜 CSMA/CD를 이용해서 반이중 통신을 했나요? 전이중 통신을 이용하면 허브로 만들어진 콜리전 도메인이 없어질 것 같은데요?

현재 UTP 케이블은 송수신선으로 전이중 통신이 가능했지만 이너뎃 초기에는 그러지 못했다. 1990년에 IEEE 802.3i의 10BASE-T라는 표준으로 UTP 케이블이 등장했다. 이때도 송수신선이 따로 있었지만 송신할 때는 수신을 하지 못해 논리적으로는 반이중 통신만 가능했다.

> 🙋🏻 송수신선이 따로면 그냥 전이중 통신하면 되는데 왜 그렇게 했나요?

정확한 이유는 찾지 못했지만 내 추측으로는 다음과 같은 상황 때문이라고 생각한다. UTP 케이블로 송수신을 동시에 하는 상황으로 허브에 UTP 케이블로 2대의 컴퓨터가 연결되어 있다고 가정하자. 여기서 PC1과 PC2가 서로에게 데이터를 동시에 전송한다면 각각의 데이터가 허브에 도착할 것이다. 허브는 PC1에게 받은 데이터를 PC1을 제외한 모든 PC로 브로드캐스팅을 한다. 마찬가지로 PC2에게 받은 데이터는 PC2를 제외한 모든 PC로 브로드캐스팅을 한다. 이렇게 UTP 케이블의 송수신선을 나눠서 전이중 통신이 성공적으로 이루어진다. 하지만 문제는 더 많은 PC가 연결된 상황에서 발생한다. 하나의 PC가 더 연결되어 PC1과 PC2가 동시에 데이터를 전송하는 상황으로 가정하자. 허브는 PC1에서 받은 데이터를 PC1을 제외한 모든 PC로 브로드캐스팅을 한다. PC2와 PC3으로 말이다. 마찬가지로 PC2에서 받은 데이터는 PC2를 제와한 모든 PC로 브로드캐스팅한다. PC1과 PC3으로 말이다. 여기서 PC3으로 가는 UTP 케이블의 수신선에 충돌이 발생한다. PC1에서 온 데이터와 PC2에서 온 데이터가 동시에 전송되니 충돌이 발생한 것이다.

> 🙋🏻 그러면 PC1 데이터를 먼저 보내고 PC2를 보내면 될 것 같은데요?

맞다. 그렇게 하면 충돌이 발생하지 않는다. 하지만 그렇게 하려면 나중에 보낼 데이터를 기억할 메모리와 처리장치가 필요한데 허브는 가지고 있지 않다. 허브는 단순히 들어온 신호를 모든 PC에게 브로드캐스팅을 해주는 포트가 많은 리피터 역할이기 때문이다. 따라서 송수신선이 따로 있는 UTP 케이블이더라도 논리적으로 반이중 통신을 선택해야만 했다. 이후에 브리지, 스위치가 만들어지면서 조금 전에 말한 방법이 가능해졌고 1997년 IEEE 802.3x에서 전이중 통신 표준이 발표되었다. 드디어 UTP 케이블의 송수신선을 제대로 이용할 수 있게 되었다. 현재는 전이중 통신을 이용하기 때문에 CSMA/CD는 거의 사용하지 않는 고대유물로 생각하면 된다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## 브리지

동축 케이블을 이용해 버스 토폴로지로 통신하면 CSMA/CD 방식으로 반이중 통신만 가능하다는 단점이 있다. UTP 케이블과 허브를 이용해 스타 토폴로지를 구성해도 마찬가지다. 허브에서 충돌이 발생할 수 있기 때문이다. 여기서 사람들은 이런 생각을 한다.

> 💁🏻 PC1과 PC2에서 데이터가 허브에 동시에 들어왔을 때 PC1의 데이터는 바로 보내고 PC2의 데이터는 메모리로 보관했다가 PC1 데이터가 전송되고 보내도 되지 않을까?

허브를 이렇게 업그레이드 한다면 충돌은 발생하지 않는다. 기존 허브는 들어 온 데이터를 브로드캐스팅하는 기능만 있지만 이제 메모리와 이를 처리하는 프로세서가 필요하다. 여기서 이런 아이디어가 떠오른다.

> 💁🏻 메모리와 프로세서를 넣겠다면 모든 포트로 브로드캐스팅을 할 이유는 없는데? 목적지의 MAC 주소를 보고 해당 MAC주소에만 데이터를 전송하면 네트워크 혼잡도가 줄어들겠는데?!

굉장히 좋은 아이디어이다. 이런 아이디어로 허브에 메모리와 CPU를 추가해 새로 만들어 낸 것이 브리지이다. 외관상 허브와 차이는 없다. 브리지는 허브에서 생긴 콜리전 도메인을 나눌 수 있다. 브리지에 4대의 컴퓨터가 연결되어 있다고 가정하자. PC1에서 PC4로 데이터를 전송한다면 목적지 MAC주소를 PC4로 설정해 브리지로 이동한다. 여기서 만약 허브였다면 허브와 연결된 모든 PC가 콜리전 도메인이므로 모든 PC로 데이터를 브로드캐스팅하겠지만 브리지는 PC1이 보낸 데이터에서 목적지 MAC주소가 PC4인 것을 확인하고 메모리에 있는 MAC주소 테이블을 참조해 PC4에만 데이터를 전송한다. PC2와 PC3은 불필요한 데이터를 받지 않아서 네트워크 트래픽이 적어진다. 또한 충돌도 발생하지 않아서 CSMA/CD 방식을 쓰지 않아도 되기 때문에 전이중 통신이 가능해진다. 만약 브리지의 한 포트를 허브와 연결해서 이런 구조를 만들었다면 콜리전 도메인은 브리지의 포트별로 나뉘게 된다. 브리지는 허브의 기능에서 MAC주소를 이용해 해당 포트로만 전송할 수 있기 때문에 이렇게 콜리전 도메인을 나눌 수 있는 것이다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## 스위치

이번 포스트에는 스위치에 대해 알아보자. 사실 스위치는 브리지와 기능적인 차이는 없다. 단지 성능이 조금 더 좋아진 것이다. 스위치는 스위칭 허브라고도 불린다. 브리지, 스위치, 스위칭 허브는 다 똑같은 말이다. 스위치와 브리지는 MAC주소를 이용하기 때문에 데이터링크 계층에 속한다. 이전 포스트에 브리지, 즉 스위치의 동작을 간단하게 살펴봤는데 이번 포스트엔 조금 더 자세히 알아보자.

4대의 PC가 허브와 스위치로 연결되어 있다고 가정하자. PC1, PC2는 허브에 연결되어 있고 이 허브와 PC3, PC4는 스위치에 연결되어 있다고 하자. 이 PC들은 이전에 통신한 적이 없다. 따라서 스위치의 메모리에 있는 MAC 주소 테이블은 비어있다. 이 상태에서 PC1이 PC4에게 데이터를 보내는 상황이다. 그럼 가장 먼저 PC1이 허브로 데이터를 전송할 것이다. 그다음은 PC2와 스위치로 데이터를 유니캐스팅한다. 여기서 PC2는 데이터의 목적지 MAC 주소를 보고 자신의 주소가 아니므로 버린다. 스위치는 어떨까? 데이터의 목적지 MAC주소가 PC4이니까 PC4에게만 전송할까? 안타깝게도 처음부터 스위치가 그렇게 동작하지 않는다. 어느 포트의 어떤 MAC주소가 있는지 모르기 때문이다. 이 네트워크에서 통신은 처음 일어난 것이라 스위치의 MAC주소 테이블은 비어있다. 가장 먼저 스위치는 허브에 전달된 데이터에서 출발지 MAC주소를 보고 MAC주소 테이블에 저장한다. 여기서 허브는 스위치의 0번 포트에 연결되었다고 가정하자. 그러면 MAC주소 테이블엔 PC1의 MAC주소와 0번 포트가 저장된다. 이 작업은 주소를 배웠다고 해서 Learning이라고 부른다. 이제 스위치는 데이터의 목적지 MAC주소를 보고 어느 포트로 보내야 하는지 판단한다. 하지만 MAC주소 테이블을 봤더니 목적지 MAC주소인 PC4는 등록되어 있지 않았다. 따라서 허브와 연결된 0번 포트를 제외한 나머지 포트로 유니캐스팅을 한다. 이 작업을 Flooding이라고 한다. 그럼 PC3과 PC4에 데이터가 도착하고 PC3은 자신에게 온 데이터가 아니므로 버린다. PC4는 자신에게 온 데이터이므로 상위계층으로 보내 데이터를 처리한다. 이번엔 PC4가 PC1로 응답하는 상황으로 가정하자. 가정 먼저 스위치로 데이터를 보낸다. 그러면 스위치는 데이터 출발지 MAC주소를 보고 Learning 한다. 즉 PC4의 MAC주소가 몇 번 포트에 연결되어 있는지 MAC주소 테이블에 기록한다. 지금은 PC4가 2번 포트에 연결되어 있다고 가정해 2로 기록한다. 그리고 스위치는 데이터 목적지 MAC주소가 MAC주소 테이블에 있는지 확인한다. PC1의 MAC주소는 MAC주소 테이블에 있으므로 유니캐스팅 즉, Flooding 하지 않고 데이터를 0번 포트로 전송한다. 이 작업을 Forwarding이라고 한다. 그러면 허브로 데이터가 전송되고 허브는 유니캐스팅으로 PC1과 PC2에게 데이터를 전송한다. PC2는 자신에게 온 데이터가 아니기 때문에 버리고 PC1은 자신에게 온 데이터이므로 상위계층으로 보내 데이터를 처리한다.

이번엔 PC2가 PC1에게 데이터를 전송한다고 가정하자. 먼저 PC2가 허브로 데이터를 전송한다. 그리고 허브는 유니캐스팅으로 PC1, 스위치에게 데이터를 전송한다. 이 때 허브로부터 데이터를 받은 PC1은 자신에게 온 데이터인걸 확인하고 상위계층으로 보내서 처리한다. 한편 스위치는 데이터 출발지 MAC주소를 Learning 하고 데이터의 목적지 MAC주소를 MAC주소 테이블에서 찾는다. 여기서 PC1과 PC2는 0번 포트로 같은 포트라는 것을 알아차린다. 따라서 PC1과 PC2는 스위치가 없어도 통신할 수 있다고 판단하고 어디에도 데이터 전송은 하지 않는다. 즉 포트 1, 포트 2번에 데이터가 흘러가지 않도록 한다. 이 작업을 Filtering이라고 한다.

스위치는 이렇게 Learning, Flooding, Forwarding, Filtering을 해서 데이터를 이동한다. 마지막으로 Aging이라는 작업이 있다. 스위치의 MAC주소 테이블에 연결된 기기의 정보를 Learning한다고 했다. 만약 시간이 오래 지나서 많은 기기의 정보가 저장되었다면 메모리 부족이 나타날 것이다. 따라서 일정시간이 지나면 메모리에서 자동으로 제거하는데 나이가 들어 죽는다는 의미로 Aging이라고 부른다. 보통 5분이 지나면 자동삭제된다.

> 🙋🏻 MAC주소 테이블에서 제거되면 다시 전송은 어떻게 해요?

위의 질문의 답으로는 Flooding과 Learning을 해서 다시 MAC주소 테이블에 등록한다.

> 🙋🏻 자주 통신하는 상황에서는 이 방법이 비효율적이지 않나요?

그렇지 않다. Aging 타이머가 다 되기 전에 MAC주소를 참조한다면 해당 MAC주소의 Aging 타이머는 초기화되어 시간이 연장된다.

> 🙋🏻 포스트 초반에 브리지와 스위치의 기능은 똑같고 성능의 차이가 있다고 했는데 어떤 차이가 있나요?

스위치는 브리지보다 더 많은 포트를 가지고 있다. 그리고 ASIC이라는 하드웨어로 스위칭 작업을 수행하므로 소프트웨어적으로 처리하는 브리지보다 속도가 더 빠르다. 스위치는 오류를 체크하는 기능이 있지만 브리지에는 없다. 마지막으로 스위치는 버퍼를 가지고 있지만 브리지는 없는 경우도 있다. 요즘엔 스위치의 가격이 많이 인하되어 브리지를 사용하는 경우는 없고 브리지와 경계가 사라져서 스위치라는 말을 많이 사용한다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## \[네트워크 구성\] 더 효율적으로 나누기

이번 포스트에는 스위치를 이용해 네트워크를 확장해 보자. PC1, PC2는 허브에 PC3과 PC4는 스위치에 허브와 스위치가 서로 연결되어 있다고 가정하자. 여기서 PC1이 웹 브라우저로 PC4에게 HTTP 요청을 한다면 어떻게 될까? 가장 먼저 프레임이 허브로 이동할 것이다. 그리고 허브는 PC1을 제외한 다른 모든 포트로 유니캐스팅을 하기 때문에 PC2와 스위치로 데이터를 전송할 것이다. PC2는 자신에게 온 데이터가 아니므로 버린다. 이 네트워크에서는 통신이 이루어진 적이 없으므로 스위치의 MAC 주소 테이블은 비어있다. 여기서 PC1에서 온 데이터를 보고 Learning 한다. 즉 스위치의 MAC 주소 테이블에 PC1의 MAC주소와 포트를 기록한다. 이제 데이터를 목적지 MAC 주소로 전송해야 하므로 MAC 주소 테이블을 참조하지만 PC4의 MAC주소는 기록되지 않았다. 따라서 스위치는 Flooding을 하여 PC3과 PC4에게 데이터를 전송한다. PC3은 자신에게 온 데이터가 아니므로 버리고 PC4는 자신에게 온 데이터이므로 상위계층에 전달해 처리한다. 그리고 서버의 처리결과를 PC1에게 전송한다. 가장 먼저 스위치로 이동하고 스위치는 PC4의 MAC 주소를 Learning 한다. 그리고 MAC 주소 테이블에서 PC1의 주소를 알고 있으므로 0번 포트 즉 허브가 연결된 포트로 forwarding 한다. 허브에서는 유니캐스팅으로 PC1과 PC2에게 전송한다. PC2는 자신에게 온 데이터가 아니므로 버리고 PC1은 자신에게 왔기 때문에 데이터를 상위계층에 전달해 처리한다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## 스패닝 트리 프로토콜

회사가 성장해 직원이 더 많아져서 스위치 1대로는 감당이 힘든 상황이라 가정하자. 더 많은 스위치를 구현해 위의 사진처럼 네트워크를 구성했다. 스위치가 이런 모양으로 구성되어 있다면 통신이 잘 될까?

> 🙋🏻 이렇게 말하니 잘 안 될 것 같아요!

맞다. 이 구조로 만들었더니 생각지 못한 치명적인 에러가 발생한다. PC1이 브로드캐스팅을 하거나 스위치 1의 MAC 주소 테이블에 목적지 MAC주소가 없어서 Flooding을 하는 상황이라고 가정하자. 그러면 스위치 1은 데이터가 들어온 포트를 제외한 나머지 포트로 데이터를 Flooding 한다. 스위치 2와 스위치 3으로 보내는 것이다. 스위치 2와 스위치 3은 받은 데이터를 마찬가지로 Flooding 한다. 그러면 PC2와 PC3은 자신에게 온 데이터이므로 상위계층에서 처리하고 스위치 4는 스위치 2와 스위치 3으로부터 받은 2개의 데이터를 Flooding 한다. 그러면 PC4는 자신에게 온 데이터이므로 상위계층에서 처리하고 스위치 1은 또다시 Flooding 한다. 이렇게 계속 스위치의 루프를 끊임없이 돌게 된다. 그러면 이런 메시지들이 계속 쌓이게 되고 이들을 처리하느라 속도가 느려져 결국 네트워크가 다운되는 상황까지 가게 된다. 이 문제는 어떻게 해결할까?

> 🙋🏻 스위치가 루프를 만들지 않도록 연결 1개만 끊어주면 되는거 아닌가요?

맞다. 연결된 4대의 스위치에 사이클이 생기지 않도록 한쪽 연결을 끊어주면 된다. 하지만 사람 손으로 일일이 하기엔 번거로워서 이 작업을 스위치가 대신해준다. 바로 스패닝 트리 프로토콜이라는 것을 이용해서 말이다. 스패닝 트리란 트리 자료구조에서 모든 노드를 사이클 없이 연결하는 트리를 말한다.  여기서 노드는 스위치로 생각하면 된다.

> 스패닝 트리: 모든 스위치를 사이클 없이 연결하는 구조

스패닝 트리는 여러개가 될 수 있다. 지금 상황에선 '스위치 1 -> 스위치 2 -> 스위치 4 -> 스위치 3'인 스패닝 트리 1개와 '스위치 1 -> 스위치 3 -> 스위치 4 -> 스위치 3'인 스패닝 트리, '스위치 3 -> 스위치 1 -> 스위치 2 -> 스위치 4'인 스패닝 트리, '스위치 2 -> 스위치 1 -> 스위치 3 -> 스위치 4'인 스패닝 트리 1개가 존재한다. 스패닝 트리 프로토콜은 여러 스패닝 트리(신장 트리)를 구하고 여기서 가장 빨리 도달할 수 있는 스패닝 트리, 즉 최소 스패닝 트리로 데이터를 전송해 무한루프에 빠지지 않게 하는 프로토콜이다. 최소 스패닝 트리를 구하는 방법은 여러 알고리즘이 있는데 최소 신장 트리가 대표적이다. 스위치는 최소 스패닝 트리를 구해서 순환구조를 제거하기 때문에 무한 브로드캐스팅 문제는 발생하지 않는다.

> 이 포스트는 [감자](https://www.inflearn.com/course/%EA%B7%B8%EB%A6%BC%EC%9C%BC%EB%A1%9C-%EC%89%BD%EA%B2%8C-%EB%B0%B0%EC%9A%B0%EB%8A%94-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC# '감자')님의 강의를 참조하여 작성하였습니다.

## 스위치의 한계

우리는 케이블을 이용해 2대의 컴퓨터를 연결하는 것을 시작으로 허브, 스위치까지 이용해 꽤 큰 네트워크를 만들 수 있게 되었다.

> 🙋🏻 지금까지 배운 스위치만 이용하면 전 세계 모든 PC를 연결할 수 있지 않나요?

스위치끼리 무한대로 연결하면 세상 모든 컴퓨터를 연결해도 문제가 없어 보인다. 하지만 스위치에 컴퓨터가 무한정으로 연결되면 결국 문제가 발생한다. 여러 대의 컴퓨터를 허브로 연결한다면 허브는 연결된 모든 컴퓨터에 유니캐스트 한다. 허브로 연결되면 충돌이 발생할 수 있는데 이 영역을 콜리전 도메인이라고 부른다. 스위치는 허브의 단점인 콜리전 도메인을 나눌 수 있다. 스위치는 목적지 MAC 주소를 보고 유니캐스팅하지 않고 목적지에만 데이터를 전송한다. 하지만 스위치가 전혀 유니캐스팅을 하지 않는다고 말하긴 어렵다. 어느 한 PC에서 브로드캐스팅 메시지를 보내거나 목적지 MAC 주소를 모르는 경우에 Flooding을 해서 Learning을 해야 한다. 따라서 스위치와 연결된 영역을 브로드캐스트 도메인이라고 부른다. 브로드캐스트 도메인은 근거리 네트워크 즉, LAN(Local Area Network) 영역이라고 부른다. 여기서 스위치를 연결해 네트워크 크기를 늘리면 어떻게 될까? 콜리전 도메인은 나눠지지만 하나의 브로드캐스트 도메인에 있는 것은 달라지지 않는다. 이렇게 스위치를 계속 연결해 수십만 대의 PC가 브로드캐스트 영역에 있게 되면 문제가 발생한다. 어느 한 PC에서 전송한 데이터의 목적지 MAC 주소가 스위치의 MAC 주소 테이블에 없다면 Flooding이 발생하고 이 메시지는 수십만 대의 모든 PC로 전송될 것이다. 목적지 PC만 자신에게 온 데이터이므로 받은 데이터를 처리하고 나머지 PC는 필요 없는 데이터이므로 모두 버린다. 통신하는 컴퓨터가 많을수록 브로드캐스트 영역에는 쓸모없는 트래픽이 많아질 것이다.

> 🙋🏻 그러면 브로드캐스트 영역을 나누면 되지 않을까요?

맞다. 우리는 이 브로드캐스트 영역을 나누기 위해 네트워크 계층을 배워야 한다.
